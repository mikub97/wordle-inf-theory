            ...
            if attr.startswith("__") or attr == "RESET" or not attr.isupper():
                ...
                for key in getattr(c, attr):
                    if parser.has_option(attr, str(key)):
                        getattr(c, attr)[key] = ("\x1b[{v}m" if attr == "STATE_COLOURS" else "{v}").format(
                            v=parser.get(attr, str(key)))
...
class CLIPlayer:
    ...
    def start(self):
        self._lines_since_keyboard = -1
        self._response_history = []
        self.keyboard_status = {letter: LetterStates.NOTGUESSEDYET for letter in string.ascii_uppercase}
    ...
    def guess(self, round) -> str:
        ...
        sys.stdout.write(
            f"\033[A\033[{len(prompt)}C\033[K")  # move cursor up one line, right by len(prompt), then clear rest of line
        return guess
    ...
    def handle_response(self, guess: str, states: List[LetterStates], hint: int):
        ...
        for letter, state in zip(guess, states):
            ...
            if state.value > self.keyboard_status[letter].value:
                self.keyboard_status[letter] = state
        self.out(
            self.pretty_response(guess, states, self._C) + (f" {self._C.DIM}{hint} possible" if hint != -1 else ""))
        self.update_keyboard()
    ...
    def update_keyboard(self):
        ...
        sys.stdout.write(self.pretty_response(list(self.keyboard_status.keys()), list(self.keyboard_status.values()),
                                              self._C) + "\xA0")
        if self._lines_since_keyboard >= 1:
    ...
    def try_clipboard(text) -> bool:
        try:
            ...
            if clip == None:
                return False
            p = subprocess.Popen(clip, stdin=subprocess.PIPE, stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT,
